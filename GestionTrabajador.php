...
if ($_SERVER["REQUEST_METHOD"] == "POST" && (!isset($_POST['form_type']) || $_POST['form_type'] == 'trabajador')) {
    ...
    $paterno = $conexion->real_escape_string(trim($_POST['paterno']));
    $materno = $conexion->real_escape_string(trim($_POST['materno']));
    ...
    $rol = isset($_POST['rol']) ? $conexion->real_escape_string(trim($_POST['rol'])) : 'trabajador';
...
// Procesar formulario de registro/edición de laboratorios
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['form_type']) && $_POST['form_type'] == 'laboratorio') {
    // Limpiar y validar datos
    $nombre_lab = $conexion->real_escape_string(trim($_POST['nombre_lab']));
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['form_type']) && $_POST['form_type'] == 'insumo') {
    // Limpiar y validar datos
    $nombre_insumo = $conexion->real_escape_string(trim($_POST['nombre_insumo']));
    $descripcion = $conexion->real_escape_string(trim($_POST['descripcion']));
    $codigo_barras = $conexion->real_escape_string(trim($_POST['codigo_barras']));
...
if (!$resultado_laboratorios) {
    echo "Error en consulta de laboratorios: " . $conexion->error;
} else {
    echo "<!-- Laboratorios encontrados: " . $resultado_laboratorios->num_rows . " -->";
}
...
// DEBUG: Verificar si hay resultados
if (!$resultado_insumos) {
    echo "Error en consulta de insumos: " . $conexion->error;
} else {
    echo "<!-- Insumos encontrados: " . $resultado_insumos->num_rows . " -->";
}
            ...
            <div class="content">
                ...
                <!-- Contenido de Gestión de Laboratorios -->
                    <?php elseif ($pagina_actual == 'laboratorios'): ?>
                        ...
                        <div class="form-section">
                            ...
                            <form method="POST" action="GestionTrabajador.php?pagina=laboratorios" class="registro-form">
                                ...
                                <div class="form-row">
                                  ...
                                  <div class="form-group">
                                    ...
                                    <select id="persona_id" name="persona_id" required>
                                        ...
                                        <?php 
                                        // Usar la variable que ya existe en lugar de hacer nueva consulta
                ...
                <?php elseif ($pagina_actual == 'insumos'): ?>
                    ...
                    <div class="form-section">
                        ...
                        <form method="POST" action="GestionTrabajador.php?pagina=insumos" class="registro-form">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group">
                                <label for="codigo_barras">Código de Barras *</label> <!-- CAMBIADO A OBLIGATORIO -->
                                ...
                                <input type="text" id="codigo_barras" name="codigo_barras" 
                                    ...
                                    required maxlength="50" placeholder="Ingrese el código de barras"> <!-- AGREGADO REQUIRED -->
                        ...
                        <div class="form-row">
                            <div class="form-group full-width">
                                ...
                                <!-- CAMBIADO A 5 FILAS EN VEZ DE 3 Y MÁXIMO 500 CARACTERES -->
                    ...
                    <div class="table-section">
                       ...
                       <!-- Barra de búsqueda y filtros -->
                        ...
                        <div class="table-container">
                            <?php 
                            // Verificar si hay resultados
                            if ($resultado_insumos && $resultado_insumos->num_rows > 0): 
                            ?>
...
<?php
// Y aquí al final sí puedes cerrarla si quieres
$conexion->close();
?>